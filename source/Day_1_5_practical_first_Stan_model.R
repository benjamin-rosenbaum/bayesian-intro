...
# which contains a deterministic part (a+b*x) and a stochastic part (y~normal(..., sigma).)
...
#------------------------------------------------------------------------------
# usually, we run more than 1 MCMC chain. These calculations are totally independent from each other and can be run simultaneously. 
...
# Now we can start the MCMC sampler using sampling(). We hand over the compiled object and the data. 
...
# More options see help function ?sampling. One important option we didn't use is specifying initial values for the chains. 
...
# Printing the results gives is information on the marginal (i.e. 1d / single parameter) estimates (remember, the posterior distribution is multidimensional):
